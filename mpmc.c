...
void mpmc_queue_register(mpmc_t *q, handle_t *th, int flag)
{
    ...
    if (flag & ENQ) {
         ...
         * for over the thread sum => i >= thread_sum
         */
        for (int i = 0;; ++i) {
            handle_t *init = NULL;
            // atomic_compare_exchange(type *ptr, type *expected, type desired)
            if (!tail[i] &&
...
static void *mpmc_find_cell(node_t *volatile *ptr, long i, handle_t *th)
{
    ...
    for (long j = curr->id; j < i / N; ++j) {
        ...
        if (!next) { /* start filling */
            ...
            // printf("j %ld, curr->id %ld, i = %ld\n", j, curr->id, i);

            tmp->id = /*NNN*/ j + 1; /* next node's id */
    ...
    *ptr = curr; /* update our node to the present node */
    ...
    return &curr->cells[/*CCC*/ i % N];
}

/**
 * futex and producer and consumer
 */
...
// v = 1 + i + ((int) index) * COUNTS_PER_THREAD
...
void *mpmc_dequeue(mpmc_t *q, handle_t *th)
{
    ...
    long index = __atomic_fetch_add(&q->pop_index, /*DDD*/ 1, __ATOMIC_SEQ_CST);

    // printf("index %ld, pop_index %ld\n", index, (q->pop_index));

    /* locate the needed cell */
    void *volatile *c = mpmc_find_cell(&th->pop_node, index, th);

    // printf("dequeue from cell\n");
...
{
    pthread_barrier_init(&prod_barrier, NULL, THREAD_NUM );
    pthread_barrier_init(&cons_barrier, NULL, THREAD_NUM );
    if (argc >= 3) {
